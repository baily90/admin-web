...
const deptTree = ref<Tree[]>([]) // 医院树形结构化
const deptList = ref<any[]>([]) // 保存扁平化的医院列表数据
const userList = ref<UserApi.UserVO[]>([]) // 所有用户列表
const filteredUserList = ref<UserApi.UserVO[]>([]) // 当前医院过滤后的用户列表
const selectedUserIdList: any = ref([]) // 选中的用户列表
const dialogVisible = ref(false) // 弹窗的是否展示
const formLoading = ref(false) // 表单的加载中
const activityId = ref()

/** 计算属性：合并已选择的用户和当前医院过滤后的用户 */
...
const open = async (id: number, selectedList?: any[]) => {
  ...
  deptList.value = deptData // 保存扁平结构的医院数据
